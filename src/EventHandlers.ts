...
Base_SpokePoolContract.AdminChanged.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...
    base_SpokePool_AdminChangedCount:
      currentSummaryEntity.base_SpokePool_AdminChangedCount + BigInt(1),
  };
...
Base_SpokePoolContract.BeaconUpgraded.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_BeaconUpgradedCount:
      currentSummaryEntity.base_SpokePool_BeaconUpgradedCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_BeaconUpgraded.set(
    base_SpokePool_BeaconUpgradedEntity
  );
});
Base_SpokePoolContract.EmergencyDeleteRootBundle.loader(
  ({ event, context }) => {
    context.EventsSummary.load(GLOBAL_EVENTS_SUMMARY_KEY);
  }
);

Base_SpokePoolContract.EmergencyDeleteRootBundle.handler(
  ({ event, context }) => {
    const summary = context.EventsSummary.get(GLOBAL_EVENTS_SUMMARY_KEY);
    ...
    const nextSummaryEntity = {
      ...currentSummaryEntity,
      base_SpokePool_EmergencyDeleteRootBundleCount:
        currentSummaryEntity.base_SpokePool_EmergencyDeleteRootBundleCount +
        BigInt(1),
    };
    ...
    context.EventsSummary.set(nextSummaryEntity);
    context.Base_SpokePool_EmergencyDeleteRootBundle.set(
      base_SpokePool_EmergencyDeleteRootBundleEntity
    );
  }
);
...
Base_SpokePoolContract.EnabledDepositRoute.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_EnabledDepositRouteCount:
      currentSummaryEntity.base_SpokePool_EnabledDepositRouteCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_EnabledDepositRoute.set(
    base_SpokePool_EnabledDepositRouteEntity
  );
});
Base_SpokePoolContract.ExecutedRelayerRefundRoot.loader(
  ({ event, context }) => {
    context.EventsSummary.load(GLOBAL_EVENTS_SUMMARY_KEY);
  }
);

Base_SpokePoolContract.ExecutedRelayerRefundRoot.handler(
  ({ event, context }) => {
    const summary = context.EventsSummary.get(GLOBAL_EVENTS_SUMMARY_KEY);
    ...
    const nextSummaryEntity = {
      ...currentSummaryEntity,
      base_SpokePool_ExecutedRelayerRefundRootCount:
        currentSummaryEntity.base_SpokePool_ExecutedRelayerRefundRootCount +
        BigInt(1),
    };
    ...
    context.EventsSummary.set(nextSummaryEntity);
    context.Base_SpokePool_ExecutedRelayerRefundRoot.set(
      base_SpokePool_ExecutedRelayerRefundRootEntity
    );
  }
);
...
Base_SpokePoolContract.FilledV3Relay.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_FilledV3RelayCount:
      currentSummaryEntity.base_SpokePool_FilledV3RelayCount + BigInt(1),
  };
...
Base_SpokePoolContract.Initialized.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_InitializedCount:
      currentSummaryEntity.base_SpokePool_InitializedCount + BigInt(1),
  };
...
Base_SpokePoolContract.PausedDeposits.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_PausedDepositsCount:
      currentSummaryEntity.base_SpokePool_PausedDepositsCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_PausedDeposits.set(
    base_SpokePool_PausedDepositsEntity
  );
});
...
Base_SpokePoolContract.PausedFills.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_PausedFillsCount:
      currentSummaryEntity.base_SpokePool_PausedFillsCount + BigInt(1),
  };
...
Base_SpokePoolContract.RelayedRootBundle.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_RelayedRootBundleCount:
      currentSummaryEntity.base_SpokePool_RelayedRootBundleCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_RelayedRootBundle.set(
    base_SpokePool_RelayedRootBundleEntity
  );
});
Base_SpokePoolContract.RequestedSpeedUpV3Deposit.loader(
  ({ event, context }) => {
    context.EventsSummary.load(GLOBAL_EVENTS_SUMMARY_KEY);
  }
);

Base_SpokePoolContract.RequestedSpeedUpV3Deposit.handler(
  ({ event, context }) => {
    const summary = context.EventsSummary.get(GLOBAL_EVENTS_SUMMARY_KEY);
    ...
    const nextSummaryEntity = {
      ...currentSummaryEntity,
      base_SpokePool_RequestedSpeedUpV3DepositCount:
        currentSummaryEntity.base_SpokePool_RequestedSpeedUpV3DepositCount +
        BigInt(1),
    };
    ...
    context.EventsSummary.set(nextSummaryEntity);
    context.Base_SpokePool_RequestedSpeedUpV3Deposit.set(
      base_SpokePool_RequestedSpeedUpV3DepositEntity
    );
  }
);
...
Base_SpokePoolContract.RequestedV3SlowFill.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_RequestedV3SlowFillCount:
      currentSummaryEntity.base_SpokePool_RequestedV3SlowFillCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_RequestedV3SlowFill.set(
    base_SpokePool_RequestedV3SlowFillEntity
  );
});
...
Base_SpokePoolContract.SetHubPool.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_SetHubPoolCount:
      currentSummaryEntity.base_SpokePool_SetHubPoolCount + BigInt(1),
  };
...
Base_SpokePoolContract.SetL1Gas.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_SetL1GasCount:
      currentSummaryEntity.base_SpokePool_SetL1GasCount + BigInt(1),
  };
...
Base_SpokePoolContract.SetL2TokenBridge.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_SetL2TokenBridgeCount:
      currentSummaryEntity.base_SpokePool_SetL2TokenBridgeCount + BigInt(1),
  };
  ...
  const base_SpokePool_SetL2TokenBridgeEntity: Base_SpokePool_SetL2TokenBridgeEntity =
    {
      ...
      tokenBridge: event.params.tokenBridge + "-extra",
      eventsSummary: GLOBAL_EVENTS_SUMMARY_KEY,
    };

  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_SetL2TokenBridge.set(
    base_SpokePool_SetL2TokenBridgeEntity
  );
});
...
Base_SpokePoolContract.SetXDomainAdmin.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_SetXDomainAdminCount:
      currentSummaryEntity.base_SpokePool_SetXDomainAdminCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_SetXDomainAdmin.set(
    base_SpokePool_SetXDomainAdminEntity
  );
});
...
Base_SpokePoolContract.TokensBridged.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_TokensBridgedCount:
      currentSummaryEntity.base_SpokePool_TokensBridgedCount + BigInt(1),
  };
...
Base_SpokePoolContract.Upgraded.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_UpgradedCount:
      currentSummaryEntity.base_SpokePool_UpgradedCount + BigInt(1),
  };
...
Base_SpokePoolContract.V3FundsDeposited.handler(({ event, context }) => {
  ...
  const nextSummaryEntity = {
    ...currentSummaryEntity,
    base_SpokePool_V3FundsDepositedCount:
      currentSummaryEntity.base_SpokePool_V3FundsDepositedCount + BigInt(1),
  };
  ...
  context.EventsSummary.set(nextSummaryEntity);
  context.Base_SpokePool_V3FundsDeposited.set(
    base_SpokePool_V3FundsDepositedEntity
  );
});
